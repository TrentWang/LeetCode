...
int C4(int n, int m) {
    ...
    m = min(m, n-m);
    for(int i = 1; i <= m; ++i) {
        // ans = ans * (n-i+1) / i % mod;  <=== 這是錯的!!!!! 除法取模會讓原本可以整除的地方變成不能整除(ex: 原本 ans 有因數 3, 取完模後就沒了, 導致後面要除以 3 的時候無法整除), 要取模請參考 C5
        ans = ans * (n-i+1) / i;
    }
    return ans;
}
...
如果 [i, i+1, i+2] 是 [3p, 1p, 2p], 組合公式會是 3p/1 * 1p/2 * 2p/3 (任意兩個連續數中一定有一個 2 的倍數, 所以 1p 或 3p 其中之一必為 2 的倍數)
